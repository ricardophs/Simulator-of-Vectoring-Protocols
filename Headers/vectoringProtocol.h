#ifndef VECTORINGPROTOCOL_H
#define VECTORINGPROTOCOL_H
...
template<class A> class VectoringProtocol {
public:
    VectoringProtocol();
    ~VectoringProtocol(){}
...
private:
    ...
    std::vector<Event<A>> processAdvertise(NodeOneDest<A> &, int, A, int, bool=false);

    std::vector<Event<A>> processUpdateMessage(NodeOneDest<A> &, int, int, const A &, int);
